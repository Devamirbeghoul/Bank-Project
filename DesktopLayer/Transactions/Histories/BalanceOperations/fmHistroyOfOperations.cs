...
{
    public partial class fmHistroyOfOperations : Form
    {
        public fmHistroyOfOperations(string Input)
        {
            InitializeComponent();
            this._Input = Input;
        ...
        public fmHistroyOfOperations()
        {
            InitializeComponent();
            cbFilterBy.Items.Add("Username");
            cbFilterBy.Items.Add("AccountNumber");
        ...
        List<string> Headers = new List<string>()
            {
                "Acc.Number" , "Old", "New" , "Type" , "Time" , "By User"
            };

        List<int> Widths = new List<int>()
            {
                110 , 200 , 200 , 110 , 170 , 110
            };
        ...
        private void _PerfromUserRefresh()
        {
            ...
            {
                _TotalPages = clsBalanceOperations.GetTotalPagesBy<string>(_PageSize, "Username" , _Input) ?? 0;
                dgvRegistry.DataSource = clsBalanceOperations.GetPerPagesBy<string>(_CurrentPage, _PageSize , "Username", _Input);
            }

            else
            {
                _TotalPages = clsBalanceOperations.GetTotalPagesBy<string , string>(_PageSize, "Username" , _Input , Filter, Value) ?? 0;
                dgvRegistry.DataSource = clsBalanceOperations.GetPerPagesBy<string , string>(_CurrentPage, _PageSize, "Username", _Input, Filter, Value);
            }
        }
        ...
        private void _PerfromClientRefersh()
        {
            if (cbFilterBy.Text == "None" || Value == null)
            {
                _TotalPages = clsBalanceOperations.GetTotalPagesBy<string>(_PageSize, "AccountNumber", _Input) ?? 0;
                dgvRegistry.DataSource = clsBalanceOperations.GetPerPagesBy<string>(_CurrentPage, _PageSize, "AccountNumber", _Input);
            }

            else
            {
                _TotalPages = clsBalanceOperations.GetTotalPagesBy<string, string>(_PageSize, "AccountNumber", _Input, Filter, Value) ?? 0;
                dgvRegistry.DataSource = clsBalanceOperations.GetPerPagesBy<string, string>(_CurrentPage, _PageSize, "AccountNumber", _Input, Filter, Value);
            }
        }
        ...
        private void _RefershGenralList()
        {
            if (cbFilterBy.Text == "None" || Value == null)
            {
                _TotalPages = clsBalanceOperations.GetTotalPages(_PageSize) ?? 0;
                dgvRegistry.DataSource = clsBalanceOperations.GetAllPerPages(_CurrentPage, _PageSize);
            }

            else
            {
                _TotalPages = clsBalanceOperations.GetTotalPagesBy<string>(_PageSize, Filter, Value) ?? 0;
                dgvRegistry.DataSource = clsBalanceOperations.GetPerPagesBy<string>(_CurrentPage, _PageSize, Filter, Value);
            }
        }
        ...
        private void ShowDetailsToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            ...
            if (_Type == clsGlobal.enType.User)
            {
                ID = (int)dgvRegistry.CurrentRow.Cells[5].Value;
            ...
            {
                ID = (int)dgvRegistry.CurrentRow.Cells[0].Value;
